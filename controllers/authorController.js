...
exports.author_create_post = [
  ...
  body("authorName", "Genre name must contain at least 3 characters")
    ...
    .isLength({ min: 3 })
    .escape(),
    body('birthDate', 'Invalid date of birth').optional({ checkFalsy: true }).isISO8601().toDate(),
    body('deathDate', 'Invalid date of death').optional({ checkFalsy: true }).isISO8601().toDate(),
...
// si no existe el author
      if (search === null||Author.length === 0) {
        let newAuthor = new Author({ name: req.body.authorName, date_of_birth: req.body.birthDate, date_of_death: req.body.deathDate });
        newAuthor.save((err) => {
          if (err) {
            return next(err);
          }
